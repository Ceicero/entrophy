...
import { z } from 'zod';
import { ValidationError, env, isProduction, redisKey } from '@entrophy/core';
...
function toSessionUser(session: SessionData): SessionUser {
  return { id: session.userId, username: session.username, globalName: session.globalName, avatarUrl: session.avatarUrl };
}

function currentSid(request: { cookies: Record<string, string | undefined>; unsignCookie: (v: string) => { valid: boolean; value: string | null } }): string | null {
...
export default async function authRoutes(app: ZodFastifyInstance): Promise<void> {
  ...
  app.get('/discord/callback', AUTH_ROUTE_RATE_LIMIT, async (request, reply) => {
    ...
    if (!stateOk) {
      throw new ValidationError('This login link has expired or was already used. Please try logging in again.');
    }
  ...
  if (env.E2E_TEST_MODE && !isProduction) {
    app.post('/test-login', AUTH_ROUTE_RATE_LIMIT, async (request, reply) => {
      ...
      await app.redis.set(
        ...
        JSON.stringify([{ id: E2E_DEMO_GUILD_ID, name: 'Entrophy Demo (seed)', icon: null, owner: true, permissions: '8' }]),
        'EX',
