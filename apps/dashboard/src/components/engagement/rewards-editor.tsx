    ...
    <div className="space-y-3">
      <div className="flex flex-col gap-2 sm:flex-row sm:items-end">
        <div className="w-28 space-y-1.5">
          ...
          <Input id="reward-level" type="number" min={1} value={level} onChange={(e) => setLevel(e.target.value)} placeholder="10" />
        </div>
        <div className="flex-1 space-y-1.5">
          <label className="text-xs font-medium text-muted-foreground">Role</label>
          <DiscordRoleSelect guildId={guildId} value={roleId} onChange={setRoleId} placeholder="Role to grant" />
        </div>
      ...
      {!isLoading && !error && (!rewards || rewards.length === 0) ? (
        <EmptyState title="No level rewards yet" description="Members earn a role automatically when they reach a configured level." />
      ) : null}
          ...
          {rewards.map((reward) => (
            <li key={reward.id} className="flex items-center justify-between gap-3 px-3 py-2">
              <span className="text-sm">
                Level <span className="font-medium tabular-nums">{reward.level}</span> → role <code className="text-xs text-muted-foreground">{reward.roleId}</code>
              </span>
              <IconButton
                label="Remove reward"
                variant="ghost"
