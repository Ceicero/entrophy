  ...
  return (
    <div className="space-y-6">
      ...
      <Card>
        ...
        <CardContent className="grid grid-cols-1 gap-4 sm:grid-cols-2">
          <FormField label="Default mode" hint="Used by /ticket open and any panel that doesn't override it.">
            <Select value={draft.mode} onValueChange={(v) => set('mode', v as Draft['mode'])} disabled={update.isPending}>
              <SelectTrigger>
                <SelectValue />
              </SelectTrigger>
              <SelectContent>
          ...
          <FormField label="Default category" hint="Parent category for channel-mode tickets.">
            <DiscordChannelSelect guildId={guildId} value={draft.categoryId} onChange={(v) => set('categoryId', v)} disabled={update.isPending} />
          </FormField>
          <FormField label="Default support roles" className="sm:col-span-2">
            <MultiRolePicker guildId={guildId} value={draft.supportRoleIds} onChange={(v) => set('supportRoleIds', v)} disabled={update.isPending} />
          </FormField>
      ...
      <Card>
        ...
        <CardContent className="grid grid-cols-1 gap-4 sm:grid-cols-2">
          <FormField label="Default response SLA (minutes)" hint="Blank/0 disables the default SLA. Panels can override this.">
            <Input
              type="number"
              min={0}
          ...
          <FormField label="Alert channel" hint="SLA-breach alerts are also posted here (in addition to the ticket itself).">
      ...
      <Card>
        ...
        <CardContent className="grid grid-cols-1 gap-4 sm:grid-cols-2">
          <FormField label="Transcript channel" hint="Closing summary + HTML transcript are posted here.">
            <DiscordChannelSelect guildId={guildId} value={draft.transcriptChannelId} onChange={(v) => set('transcriptChannelId', v)} disabled={update.isPending} />
          </FormField>
          ...
          <div className="flex items-start justify-between gap-4 rounded-md border border-border p-3 sm:col-span-2">
            <div>
              ...
              <p className="text-xs text-muted-foreground">Sent on close, in addition to the transcript channel.</p>
            </div>
            <Switch checked={draft.dmTranscript} onCheckedChange={(v) => set('dmTranscript', v)} disabled={update.isPending} />
          </div>
        </CardContent>
      </Card>
      ...
      <Card>
        ...
        <CardContent className="space-y-4">
          <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
            <FormField label="Reopen window (hours)" hint="How long after close a ticket can be reopened. 0 disables reopening.">
              <Input
                type="number"
                min={0}
                max={720}
            ...
            <FormField label="Delete closed channel after (seconds)" hint="Only applies to channel-mode tickets, and only when channels aren't kept.">
              <Input
                type="number"
                min={0}
          ...
          <div className="flex items-start justify-between gap-4 rounded-md border border-border p-3">
            <div>
              ...
              <p className="text-xs text-muted-foreground">Closed channels stay (with the opener's access removed) instead of being deleted.</p>
            </div>
            <Switch checked={draft.keepClosedChannels} onCheckedChange={(v) => set('keepClosedChannels', v)} disabled={update.isPending} />
          </div>
        </CardContent>
      </Card>
      ...
      <Card>
        ...
        <CardContent>
          <p className="mb-3 text-xs text-muted-foreground">
            Asked in a modal when a ticket is opened from a panel with the &quot;intake&quot; toggle on. New panels snapshot this list at creation time.
          </p>
          <IntakeFormBuilder value={draft.intakeForm} onChange={(v) => set('intakeForm', v)} disabled={update.isPending} />
        </CardContent>
      </Card>
    </div>
  );
