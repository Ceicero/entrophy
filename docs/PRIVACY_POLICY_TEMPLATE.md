...
| Enforcer (disabled by default) | A sanitized excerpt of a flagged message, and — only when "Capture context" is on (on by default *when Enforcer itself is enabled*) — a short snapshot of the messages immediately before the flagged one, so a moderator has context without confronting the reporting member directly | `EnforcerRecord` (`excerpt`, `contextSnapshot`) |
