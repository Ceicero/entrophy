...
export function addRecordSubcommand(builder: SlashCommandBuilder): SlashCommandBuilder {
  return builder.addSubcommand((sub) =>
    sub.setName('record').setDescription('View a ledger record in detail.').addIntegerOption((opt) => opt.setName('number').setDescription('Record number, e.g. 12 for #E-12.').setRequired(true).setMinValue(1)),
...
export async function executeRecord(c: CommandContext): Promise<void> {
  ...
  const contextSnapshot = Array.isArray(record.contextSnapshot) ? (record.contextSnapshot as { authorId: string; at: string; excerpt: string }[]) : [];
  ...
  const lines = [
    ...
    record.decision ? `Decision: ${record.decision} by <@${record.decidedBy}>${record.decisionReason ? ` — ${record.decisionReason}` : ''}` : undefined,
  ...
  await c.interaction.reply({ embeds: [infoEmbed(c.t('record.detailTitle', { number }), lines.join('\n'))], ephemeral: true });
}
