...
import { assertStaffLevel, buildCustomId, successEmbed, PendingStore, type CommandContext, type ContextMenuContext, type PluginCommand } from '../../sdk';
...
export function addFlagSubcommand(builder: SlashCommandBuilder): SlashCommandBuilder {
  return builder.addSubcommand((sub) =>
    sub
      ...
      .addStringOption((opt) => opt.setName('policy').setDescription('Related policy, if any.').setRequired(false).setAutocomplete(true)),
...
export async function executeFlag(c: CommandContext): Promise<void> {
  ...
  await c.interaction.reply({ embeds: [successEmbed(c.t('flag.created', { number: result.recordNumber }))], ephemeral: true });
}
...
export const flagContextMenuCommand: PluginCommand = {
  data: new ContextMenuCommandBuilder().setName('Flag for review').setType(ApplicationCommandType.Message).setDMPermission(false).setDefaultMemberPermissions(PermissionFlagsBits.ModerateMembers),
  ...
  async executeContextMenu(c: ContextMenuContext): Promise<void> {
    ...
    const policies = await c.ctx.prisma.enforcerPolicy.findMany({ where: { guildId: c.guildId, enabled: true, deletedAt: null }, take: 24, orderBy: { name: 'asc' } });

    const pendingStore = new PendingStore(c.ctx.redis);
    const pendingId = await pendingStore.put(
      { targetUserId: message.author.id, channelId: message.channelId, messageId: message.id, content: message.content ?? '' },
      120,
    );
    ...
    const select = new StringSelectMenuBuilder()
      ...
      .addOptions(
        ...
        ...policies.map((p) => ({ label: p.name.slice(0, 100), value: p.id, description: p.description.slice(0, 100) })),
      );
