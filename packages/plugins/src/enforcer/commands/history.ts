...
export function addHistorySubcommand(builder: SlashCommandBuilder): SlashCommandBuilder {
  return builder.addSubcommand((sub) =>
    sub.setName('history').setDescription('Summarize a user\'s Enforcer history.').addUserOption((opt) => opt.setName('user').setDescription('User').setRequired(true)),
...
export async function executeHistory(c: CommandContext): Promise<void> {
  ...
  const [flagCount, decisionCount, recent] = await Promise.all([
    ...
    c.ctx.prisma.enforcerRecord.findMany({ where: { guildId: c.guildId, userId: user.id }, orderBy: { createdAt: 'desc' }, take: 5 }),
  ]);
  ...
  await c.interaction.reply({ embeds: [infoEmbed(c.t('history.title', { userId: user.id }), lines.join('\n'))], ephemeral: true });
}
