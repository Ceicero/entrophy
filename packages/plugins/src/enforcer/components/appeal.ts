  ...
  async handler(c) {
    ...
    if (!pending) {
      await interaction.reply({
        embeds: [errorEmbed('This appeal session has expired — run `/enforcer appeal` again.')],
        ephemeral: true,
      });
      return;
    }
    ...
    if (!moderation) {
      await interaction.reply({
        embeds: [
          errorEmbed('The moderation plugin is unavailable right now — please contact staff directly.'),
        ],
        ephemeral: true,
      });
      return;
