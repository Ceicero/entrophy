import { ActionRowBuilder, ModalBuilder, TextInputBuilder, TextInputStyle, type ModalSubmitInteraction, type StringSelectMenuInteraction } from 'discord.js';
...
const selectPolicyHandler: ComponentHandler = {
  ...
  async handler(c) {
    ...
    if (!pending) {
      await interaction.update({ content: 'This flag session has expired — run "Flag for review" again.', components: [] });
      return;
    }

    const modal = new ModalBuilder()
      .setCustomId(buildCustomId('enforcer', 'flag-note-modal', ownerId ?? interaction.user.id, pendingId ?? '', policyId))
      .setTitle('Flag for review')
      .addComponents(
        new ActionRowBuilder<TextInputBuilder>().addComponents(
          new TextInputBuilder().setCustomId('note').setLabel('Note for the moderator queue (optional)').setStyle(TextInputStyle.Paragraph).setRequired(false).setMaxLength(500),
        ),
      );
...
const noteModalHandler: ComponentHandler = {
  ...
  async handler(c) {
    ...
    if (!pending) {
      await interaction.reply({ embeds: [errorEmbed('This flag session has expired — run "Flag for review" again.')], ephemeral: true });
      return;
    }
    ...
    if (resolvedPolicyId) {
      const policy = await c.ctx.prisma.enforcerPolicy.findFirst({ where: { id: resolvedPolicyId, guildId: c.guildId, deletedAt: null } });
    ...
    await interaction.reply({ embeds: [successEmbed(`Flag **#E-${result.recordNumber}** opened.`)], ephemeral: true });
  },
};
