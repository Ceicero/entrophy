...
describe('ai redact', () => {
  it('replaces user/role mentions and @everyone/@here with @user', () => {
    expect(redactMentions('hey <@123456789012345678> and <@!223456789012345678>')).toBe(
      'hey @user and @user',
    );
  ...
  it('replaces URLs but keeps the domain', () => {
    expect(redactUrls('see https://example.com/some/secret/path?x=1 for details')).toBe(
      'see [link: example.com] for details',
    );
  ...
  it('replaces token/key-shaped strings', () => {
    ...
    expect(redactTokens('Authorization: Bearer abc123.def456-ghi789')).toBe(
  ...
  it('does not redact ordinary words or short numbers', () => {
    expect(redactTokens('hello world this is a normal sentence')).toBe(
  ...
  it('composes every pass via redact()', () => {
    const input =
      'Hey <@123456789012345678>, email me at me@example.com or call 555-123-4567, see https://example.com/x, key sk-abcdefghij1234567890';
