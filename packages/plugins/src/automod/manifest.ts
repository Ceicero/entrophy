...
export const configSchema = z.object({
  ...
  defaultTimeoutMs: z.number().int().positive().max(28 * 24 * 60 * 60 * 1000).default(600_000),
...
export const manifest = defineManifest({
  ...
  defaultEnabled: true,
  permissions: [
    { permission: PermissionFlagsBits.ViewChannel, feature: 'reading messages to evaluate rules, and posting alerts', optional: false, fallback: 'Automod cannot see messages in that channel; rules never fire there.' },
    { permission: PermissionFlagsBits.SendMessages, feature: 'posting alert-staff embeds and the review queue', optional: false, fallback: 'Alert embeds fail to send; matches are still recorded and visible via /automod review and the dashboard.' },
    { permission: PermissionFlagsBits.EmbedLinks, feature: 'alert and review-queue embeds', optional: false, fallback: 'Alerts post as plain text instead of embeds.' },
    { permission: PermissionFlagsBits.ManageMessages, feature: 'the "delete" action', optional: true, fallback: 'Matching messages are flagged/logged but not deleted.' },
    { permission: PermissionFlagsBits.ModerateMembers, feature: 'the "timeout" action', optional: true, fallback: 'Matching users are flagged/logged but not timed out.' },
    { permission: PermissionFlagsBits.ManageRoles, feature: 'the "quarantine" action and raid-lockdown quarantine', optional: true, fallback: 'Quarantine actions are flagged/logged but the role is not assigned.' },
    { permission: PermissionFlagsBits.ManageGuild, feature: 'raising the verification level during a raid lockdown', optional: true, fallback: '"raise-verification" raid lockdown is skipped; the rule still alerts staff.' },
  ],
