...
const data = new SlashCommandBuilder()
  ...
  .addSubcommand((sub) =>
    sub
      ...
      .addUserOption((opt) => opt.setName('user').setDescription('Leave empty to reset every member in this server.')),
  )
    ...
    group
      ...
      .addSubcommand((sub) =>
        sub
          ...
          .addUserOption((opt) => opt.setName('user').setDescription('Member to adjust.').setRequired(true))
          .addIntegerOption((opt) => opt.setName('amount').setDescription('New XP total.').setMinValue(0).setRequired(true)),
      ),
  )
  .addSubcommandGroup((group) =>
    group
      ...
      .addSubcommand((sub) =>
        sub
          ...
          .addIntegerOption((opt) => opt.setName('level').setDescription('Level required.').setMinValue(1).setRequired(true))
      ...
      .addSubcommand((sub) =>
        sub
          ...
          .addIntegerOption((opt) => opt.setName('level').setDescription('Level of the reward to remove.').setMinValue(1).setRequired(true))
          .addRoleOption((opt) => opt.setName('role').setDescription('Role of the reward to remove.').setRequired(true)),
      )
      .addSubcommand((sub) => sub.setName('list').setDescription('List all level-role rewards.'))
      .addSubcommand((sub) => sub.setName('sync').setDescription('Recompute level-role rewards for every ranked member.')),
  )
  .addSubcommandGroup((group) =>
    group
      .setName('ignore')
      ...
      .addSubcommand((sub) =>
        sub
          ...
          .addChannelOption((opt) => opt.setName('channel').setDescription('Channel to ignore.').addChannelTypes(ChannelType.GuildText, ChannelType.GuildAnnouncement, ChannelType.GuildForum))
      ...
      .addSubcommand((sub) =>
        sub
          ...
          .addChannelOption((opt) => opt.setName('channel').setDescription('Channel to un-ignore.').addChannelTypes(ChannelType.GuildText, ChannelType.GuildAnnouncement, ChannelType.GuildForum))
...
async function fetchLeaderboardPage(ctx: PluginContext, guildId: string, page: number) {
  ...
  const [rows, total] = await Promise.all([
    ctx.prisma.levelProfile.findMany({ where: { guildId }, orderBy: { xp: 'desc' }, skip: offset, take: LEADERBOARD_PAGE_SIZE }),
...
function buildLeaderboardEmbed(
  ...
  const lines =
    rows.length > 0
      ? rows.map((r, i) => `**#${offset + i + 1}** ${userMention(r.userId)} — level ${r.level} (${r.xp.toLocaleString()} XP)`)
      : [t('level.leaderboard.empty')];
  return listEmbed(t('level.leaderboard.title', { page, totalPages }), lines);
}

function buildLeaderboardRow(ownerId: string, page: number, totalPages: number): ActionRowBuilder<ButtonBuilder> {
  return new ActionRowBuilder<ButtonBuilder>().addComponents(
    new ButtonBuilder().setCustomId(buildCustomId('engagement', 'lb-page', ownerId, String(page - 1))).setLabel('◀ Previous').setStyle(ButtonStyle.Secondary).setDisabled(page <= 1),
    new ButtonBuilder().setCustomId(buildCustomId('engagement', 'lb-page', ownerId, String(page + 1))).setLabel('Next ▶').setStyle(ButtonStyle.Secondary).setDisabled(page >= totalPages),
  );
}
...
export const command: PluginCommand = {
  ...
  async execute(c) {
    ...
    if (group === 'xp') {
      ...
      const before = await c.ctx.prisma.levelProfile.findUnique({ where: { guildId_userId: { guildId: c.guildId, userId: targetUser.id } } });
      const currentXp = before?.xp ?? 0;
      const nextXp = sub === 'give' ? currentXp + amount : sub === 'remove' ? Math.max(0, currentXp - amount) : Math.max(0, amount);
      ...
      if (leveledUp) {
        ...
        await applyLevelUp({ ctx: c.ctx, guild: c.interaction.guild, member, guildId: c.guildId, userId: targetUser.id, newLevel, config: config.leveling });
      }

      const key = sub === 'give' ? 'level.xp.given' : sub === 'remove' ? 'level.xp.removed' : 'level.xp.set';
      await c.interaction.reply({
        embeds: [successEmbed(c.t(key, { amount, user: userMention(targetUser.id), level: newLevel, xp: nextXp }))],
        ephemeral: true,
      });
      return;
    }
    ...
    if (group === 'rewards') {
      ...
      if (sub === 'add') {
        ...
        const reward = await c.ctx.prisma.levelReward.create({ data: { guildId: c.guildId, level, roleId: role.id } });
        await c.ctx.audit({ guildId: c.guildId, actorId: c.interaction.user.id, actorType: 'user', action: 'engagement.reward.create', targetType: 'level_reward', targetId: reward.id, after: { level, roleId: role.id }, source: 'bot' });
        await c.interaction.reply({ embeds: [successEmbed(c.t('level.rewards.added', { level, role: `<@&${role.id}>` }))], ephemeral: true });
        return;
      }
      ...
      if (sub === 'remove') {
        const level = c.interaction.options.getInteger('level', true);
        const role = c.interaction.options.getRole('role', true);
        const existing = await c.ctx.prisma.levelReward.findFirst({ where: { guildId: c.guildId, level, roleId: role.id } });
        if (!existing) {
        ...
        await c.ctx.audit({ guildId: c.guildId, actorId: c.interaction.user.id, actorType: 'user', action: 'engagement.reward.delete', targetType: 'level_reward', targetId: existing.id, source: 'bot' });
        await c.interaction.reply({ embeds: [successEmbed(c.t('level.rewards.removed'))], ephemeral: true });
        return;
      }

      if (sub === 'list') {
        const rewards = await c.ctx.prisma.levelReward.findMany({ where: { guildId: c.guildId }, orderBy: { level: 'asc' } });
        const lines = rewards.length > 0 ? rewards.map((r) => `Level ${r.level} → <@&${r.roleId}>`) : [c.t('level.rewards.empty')];
      ...
      const profiles = await c.ctx.prisma.levelProfile.findMany({ where: { guildId: c.guildId, level: { gt: 0 } } });
      let synced = 0;
      for (const profile of profiles) {
        const member = await fetchMemberSafe(c.interaction.guild, profile.userId);
        if (!member) continue;
        const plan = computeLevelRewardPlan([...member.roles.cache.keys()], rewards, profile.level, config.leveling.rewardMode);
        if (plan.toAdd.length === 0 && plan.toRemove.length === 0) continue;
        try {
          if (plan.toAdd.length > 0) await member.roles.add(plan.toAdd, 'Entrophy engagement: /level rewards sync');
          if (plan.toRemove.length > 0) await member.roles.remove(plan.toRemove, 'Entrophy engagement: /level rewards sync');
          synced += 1;
        } catch {
          // Missing permission/hierarchy for this member; skip and keep going.
        }
      }
      await c.interaction.reply({ embeds: [successEmbed(c.t('level.rewards.syncDone', { count: synced }))], ephemeral: true });
      return;
    }
    ...
    if (group === 'ignore') {
      ...
      if ((!channel && !role) || (channel && role)) {
        await c.interaction.reply({ embeds: [errorEmbed(c.t('errors.needExactlyOneOption'))], ephemeral: true });
        return;
      }
      ...
      if (channel) {
        ...
        await c.ctx.setConfig<EngagementConfig>(c.guildId, { leveling: { ...config.leveling, ignoredChannelIds: [...ids] } }, { id: c.interaction.user.id, source: 'bot' });
        await c.interaction.reply({ embeds: [successEmbed(c.t(adding ? 'level.ignore.addedChannel' : 'level.ignore.removedChannel', { channel: `<#${channel.id}>` }))], ephemeral: true });
        return;
      }
      if (role) {
        ...
        await c.ctx.setConfig<EngagementConfig>(c.guildId, { leveling: { ...config.leveling, ignoredRoleIds: [...ids] } }, { id: c.interaction.user.id, source: 'bot' });
        await c.interaction.reply({ embeds: [successEmbed(c.t(adding ? 'level.ignore.addedRole' : 'level.ignore.removedRole', { role: `<@&${role.id}>` }))], ephemeral: true });
        return;
      }
      return;
    }
    ...
    if (sub === 'rank') {
      ...
      const profile = await c.ctx.prisma.levelProfile.findUnique({ where: { guildId_userId: { guildId: c.guildId, userId: target.id } } });
      if (!profile) {
        await c.interaction.reply({ embeds: [brandEmbed().setDescription(c.t('level.rank.notRanked', { name: target.username }))], ephemeral: false });
        return;
      }
      const rank = 1 + (await c.ctx.prisma.levelProfile.count({ where: { guildId: c.guildId, xp: { gt: profile.xp } } }));
      ...
      const embed = brandEmbed()
        ...
        .addFields(
          ...
          { name: 'Progress', value: `${bar}\n${intoLevel.toLocaleString()} / ${span.toLocaleString()} XP to next level`, inline: false },
    ...
    const embed = brandEmbed()
      ...
      .setDescription(targetUser ? c.t('level.reset.confirmDescriptionUser', { user: userMention(targetUser.id) }) : c.t('level.reset.confirmDescriptionAll'));
    ...
    if (result.confirmed) {
      ...
      await c.interaction.reply({
        embeds: [successEmbed(result.payload.targetUserId ? c.t('level.reset.doneUser', { user: userMention(result.payload.targetUserId) }) : c.t('level.reset.doneAll', { count }))],
        ephemeral: true,
      });
    }
  },
...
async function performReset(ctx: PluginContext, guildId: string, actorId: string, payload: ResetPayload): Promise<number> {
...
const [confirmReset, cancelReset] = registerConfirmHandlers<ResetPayload>('level-reset', async (c, payload) => {
  const count = await performReset(c.ctx, c.guildId, c.interaction.user.id, payload);
  await c.interaction.followUp({
    embeds: [successEmbed(payload.targetUserId ? c.t('level.reset.doneUser', { user: userMention(payload.targetUserId) }) : c.t('level.reset.doneAll', { count }))],
    ephemeral: true,
  });
});
...
const leaderboardPageComponent: ComponentHandler = {
  ...
  async handler(c) {
    ...
    const totalPages = Math.max(1, Math.ceil(total / LEADERBOARD_PAGE_SIZE));
    const embed = buildLeaderboardEmbed(rows, offset, total, page, c.t);
    await interaction.update({ embeds: [embed], components: [buildLeaderboardRow(ownerId, page, totalPages)] });
  },
};
