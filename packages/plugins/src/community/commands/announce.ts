...
import { errorEmbed, infoEmbed, listEmbed, resolveTextChannel, successEmbed, type CommandContext, type PluginCommand } from '../../sdk';
...
const data = new SlashCommandBuilder()
  ...
  .addSubcommand((sub) =>
    sub
      ...
      .addChannelOption((opt) => opt.setName('channel').setDescription('Channel to post in').setRequired(true).addChannelTypes(ChannelType.GuildText, ChannelType.GuildAnnouncement))
      .addStringOption((opt) => opt.setName('when').setDescription('A cron expression, an ISO date/time, or a duration like "10m"/"2h"').setRequired(true))
      .addStringOption((opt) => opt.setName('message').setDescription('The message to send').setRequired(true).setMaxLength(2000)),
  )
  .addSubcommand((sub) => sub.setName('list').setDescription('List scheduled announcements.'))
  .addSubcommand((sub) =>
    sub.setName('cancel').setDescription('Cancel a scheduled announcement.').addStringOption((opt) => opt.setName('id').setDescription('Announcement id').setRequired(true).setAutocomplete(true)),
  )
  .addSubcommand((sub) =>
    sub.setName('preview').setDescription('Preview a scheduled announcement.').addStringOption((opt) => opt.setName('id').setDescription('Announcement id').setRequired(true).setAutocomplete(true)),
  );
...
async function handleSchedule(c: CommandContext): Promise<void> {
  ...
  if (parsed.kind === 'cron') {
    await queue.upsertJobScheduler(`ann:${announcement.id}`, { pattern: parsed.cron, tz: timezone }, { name: 'announcement-run', data: { announcementId: announcement.id } });
  } else {
    await queue.add('announcement-run', { announcementId: announcement.id }, { jobId: `ann:${announcement.id}`, delay: Math.max(0, parsed.runAt.getTime() - Date.now()) });
  }

  const when_ = parsed.kind === 'cron' ? t('announce.scheduledCron', { cron: parsed.cron }) : t('announce.scheduledAt', { time: parsed.runAt.toISOString() });
  await interaction.reply({ embeds: [successEmbed(`${t('announce.scheduled', { channel: `<#${channel.id}>` })} ${when_}`)], ephemeral: true });
}

async function handleList(c: CommandContext): Promise<void> {
  const { interaction, ctx, guildId, t } = c;
  const rows = await ctx.prisma.scheduledAnnouncement.findMany({ where: { guildId }, orderBy: { createdAt: 'desc' }, take: 25 });
  const lines = rows.map((a) => {
    const body = a.content as unknown as AnnouncementContent | null;
    const preview = body?.content ? body.content.slice(0, 40) : '(empty)';
    const schedule = a.cron ? `cron \`${a.cron}\`` : a.runAt ? `once at ${a.runAt.toISOString()}` : 'unscheduled';
...
async function handlePreview(c: CommandContext): Promise<void> {
  ...
  await interaction.reply({ embeds: [infoEmbed(t('announce.previewTitle'), body?.content ?? '_Empty_')], ephemeral: true });
}
...
export const command: PluginCommand = {
  ...
  async autocomplete(c) {
    ...
    const rows = await c.ctx.prisma.scheduledAnnouncement.findMany({ where: { guildId: c.guildId }, orderBy: { createdAt: 'desc' }, take: 50 });
    const matches = rows
      .filter((a) => a.id.includes(query) || ((a.content as unknown as AnnouncementContent | null)?.content ?? '').toLowerCase().includes(query))
      .slice(0, 25);
    await c.interaction.respond(
      matches.map((a) => ({ name: `${(a.content as unknown as AnnouncementContent | null)?.content ?? a.id}`.slice(0, 100), value: a.id })),
    );
  },
};
