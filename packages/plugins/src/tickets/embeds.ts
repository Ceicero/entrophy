...
import { BRAND, discordTimestamp, sanitizeEmbedText } from '@entrophy/core';
...
export function buildPanelButtons(panelId: string, buttonLabel: string): ActionRowBuilder<ButtonBuilder> {
  return new ActionRowBuilder<ButtonBuilder>().addComponents(
    new ButtonBuilder().setCustomId(buildCustomId('tickets', 'open', panelId)).setLabel(buttonLabel.slice(0, 80)).setStyle(ButtonStyle.Primary).setEmoji('🎫'),
  );
}

/** The opening embed posted in a newly-created ticket channel/thread. */
export function buildOpeningEmbed(ticket: Ticket, subject: string | null, intake: Record<string, string> | null): EmbedBuilder {
  const embed = new EmbedBuilder()
    .setColor(BRAND.color)
    .setTitle(`Ticket #${ticket.number}`)
    .setDescription(subject ? sanitizeEmbedText(subject, 4000) : 'No subject given.')
    .addFields({ name: 'Opened by', value: `<@${ticket.openerId}>`, inline: true }, { name: 'Opened', value: discordTimestamp(ticket.createdAt, 'R'), inline: true })
    .setFooter({ text: BRAND.name })
    .setTimestamp();
...
export function buildOpeningButtons(ticketId: string): ActionRowBuilder<ButtonBuilder> {
  return new ActionRowBuilder<ButtonBuilder>().addComponents(
    new ButtonBuilder().setCustomId(buildCustomId('tickets', 'close', ticketId)).setLabel('Close').setStyle(ButtonStyle.Danger).setEmoji('🔒'),
    new ButtonBuilder().setCustomId(buildCustomId('tickets', 'claim', ticketId)).setLabel('Claim').setStyle(ButtonStyle.Secondary).setEmoji('🙋'),
    new ButtonBuilder().setCustomId(buildCustomId('tickets', 'add-user', ticketId)).setLabel('Add user').setStyle(ButtonStyle.Secondary).setEmoji('➕'),
  );
}
...
export function buildReopenButton(ticketId: string): ActionRowBuilder<ButtonBuilder> {
  return new ActionRowBuilder<ButtonBuilder>().addComponents(
    new ButtonBuilder().setCustomId(buildCustomId('tickets', 'reopen-btn', ticketId)).setLabel('Reopen').setStyle(ButtonStyle.Secondary).setEmoji('🔓'),
  );
}
