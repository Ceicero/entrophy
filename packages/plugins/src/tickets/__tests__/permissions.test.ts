...
describe('buildTicketChannelOverwrites', () => {
  it('denies @everyone View Channel', () => {
    const overwrites = buildTicketChannelOverwrites({ everyoneRoleId: EVERYONE, openerId: OPENER, supportRoleIds: [], botId: BOT });
  ...
  it('gives the bot Manage Messages and Manage Channels in addition to the opener grants', () => {
    ...
    expect(bot?.allow).toEqual(expect.arrayContaining([PermissionFlagsBits.ManageMessages, PermissionFlagsBits.ManageChannels, PermissionFlagsBits.ViewChannel]));
  });

  it('never produces a duplicate overwrite id when a support role collides with the opener or bot id', () => {
    const overwrites = buildTicketChannelOverwrites({ everyoneRoleId: EVERYONE, openerId: OPENER, supportRoleIds: [OPENER, BOT, '555555555555555555'], botId: BOT });
