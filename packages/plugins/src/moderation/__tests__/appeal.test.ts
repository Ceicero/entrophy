...
function buildHarness() {
  ...
  const moderationAppeal = {
    ...
    findMany: async ({ where }: { where: { status: { in: string[] } } }) =>
      [...appeals.values()].filter((a) => where.status.in.includes(a.status)),
  };

  const moderationCase = {
    findUnique: async ({ where }: { where: { id: string } }) => cases.get(where.id) ?? null,
    updateMany: async ({ where, data }: { where: { guildId: string; targetId: string; type: string }; data: { expiredAt: Date } }) => {
      let count = 0;
      for (const row of cases.values()) {
        if (row.guildId === where.guildId && row.targetId === where.targetId && row.type === where.type && row.expiredAt === null) {
...
describe('appeal state transitions', () => {
  it('openAppeal creates a PENDING row linked to the case', async () => {
    ...
    const { appealId } = await service.openAppeal({ guildId: 'g1', userId: 'u1', caseId: 'case-1', content: 'Please reconsider.', source: 'bot' });
  ...
  it('decideAppeal(accept) on a TIMEOUT case removes the timeout and DMs the user', async () => {
    ...
    cases.set('case-1', { id: 'case-1', guildId: 'g1', caseNumber: 5, type: 'TIMEOUT', targetId: 'u1', expiredAt: null });
    const { appealId } = await service.openAppeal({ guildId: 'g1', userId: 'u1', caseId: 'case-1', content: 'x', source: 'bot' });

    const updated = await service.decideAppeal({ guildId: 'g1', appealId, accept: true, reviewerId: 'staff-1' });
  ...
  it('decideAppeal(deny) does not touch the case, but still DMs the user', async () => {
    ...
    const updated = await service.decideAppeal({ guildId: 'g1', appealId, accept: false, reviewerId: 'staff-1' });
  ...
  it('syncDashboardDecidedAppeals applies dashboard-written decisions exactly once', async () => {
    ...
    cases.set('case-1', { id: 'case-1', guildId: 'g1', caseNumber: 9, type: 'TIMEOUT', targetId: 'u1', expiredAt: null });
    const { appealId } = await service.openAppeal({ guildId: 'g1', userId: 'u1', caseId: 'case-1', content: 'x', source: 'bot' });

    // Simulate the dashboard deciding it directly in the database (no Discord side effects yet).
    const row = appeals.get(appealId)!;
    appeals.set(appealId, { ...row, status: 'ACCEPTED', reviewedBy: 'dashboard-user', reviewedAt: new Date() });
    ...
    expect(secondRun).toBe(0); // already applied — Redis marker prevents re-applying
    expect(timeoutFn).toHaveBeenCalledTimes(1);
  });
});
