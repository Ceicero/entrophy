  ...
  async handler(c) {
    ...
    if (existing?.rulesAcceptedAt) {
      await interaction.reply({
        embeds: [successEmbed("You've already agreed to the rules.")],
        ephemeral: true,
      });
      return;
    }
      ...
      if (roles) {
        await roles
          .assignRoles({
            guildId: c.guildId,
            userId: interaction.user.id,
            addRoleIds: [config.rulesRoleId],
            reason: 'Accepted server rules',
          })
          .catch(() => undefined);
      }
    }
    ...
    await c.ctx.audit({
      ...
      actorType: 'user',
      action: 'onboarding.rules.accepted',
      targetType: 'member',
    ...
    await interaction.reply({
      embeds: [successEmbed('Thanks — you have agreed to the server rules.')],
      ephemeral: true,
    });
  },
};
