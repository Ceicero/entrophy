...
function disableAllButtons(components: ButtonInteraction<'cached'>['message']['components']): ActionRowBuilder<ButtonBuilder>[] {
  ...
  return {
    ...
    async handler(c) {
      ...
      if (!request || !canDecideVerification(request.status)) {
        await interaction.reply({ embeds: [errorEmbed('This request was already decided by another moderator.')], ephemeral: true });
        return;
      }

      const roles = c.ctx.services.get('roles');
      if (!roles) {
        await interaction.reply({ embeds: [errorEmbed('The roles service is not available right now.')], ephemeral: true });
        return;
      }

      await roles.verificationDecision({ guildId: c.guildId, requestId: request.id, approve, reviewerId: interaction.user.id });

      const disabled = disableAllButtons(interaction.message.components);
      await interaction.update({ content: `${approve ? 'Approved' : 'Denied'} by <@${interaction.user.id}>.`, components: disabled });
      await interaction.followUp({ embeds: [successEmbed(`Marked ${approve ? 'approved' : 'denied'}.`)], ephemeral: true });
    },
  };
}
