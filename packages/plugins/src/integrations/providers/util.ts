 ...
 */
export async function claimAlertOnce(ctx: PluginContext, provider: string, scope: string, itemId: string, ttlSeconds = DEFAULT_DEDUPE_TTL_SECONDS): Promise<boolean> {
...
export async function sendConnectionAlert(ctx: PluginContext, connection: IntegrationConnection, embed: AlertEmbedData): Promise<boolean> {
  const config = readAlertConfig(connection);
  if (!config.channelId) return false;
  const target: AlertTarget = { guildId: connection.guildId, channelId: config.channelId, roleId: config.roleId ?? undefined };
...
export async function markConnectionError(ctx: PluginContext, connectionId: string, error: unknown): Promise<void> {
