...
export async function dispatchOutboundPlatformEvent(ctx: PluginContext, eventName: OutboundPlatformEvent, guildId: string, data: Record<string, unknown>): Promise<void> {
  const endpoints = await ctx.prisma.webhookEndpoint.findMany({ where: { guildId, direction: 'OUTBOUND', enabled: true, deletedAt: null } });
  ...
  for (const endpoint of matches) {
    await ctx.queue('outbound').add('deliver', { endpointId: endpoint.id, guildId, payload, attempt: 1 }, OUTBOUND_JOB_OPTIONS);
  }
}
...
export function registerOutboundEventBridge(ctx: PluginContext): void {
  ctx.events.on('moderation.caseCreated', (e) => void dispatchOutboundPlatformEvent(ctx, 'moderation.caseCreated', e.guildId, { ...e }));
  ctx.events.on('ticket.opened', (e) => void dispatchOutboundPlatformEvent(ctx, 'ticket.opened', e.guildId, { ...e }));
  ctx.events.on('ticket.closed', (e) => void dispatchOutboundPlatformEvent(ctx, 'ticket.closed', e.guildId, { ...e }));
  ctx.events.on('member.verified', (e) => void dispatchOutboundPlatformEvent(ctx, 'member.verified', e.guildId, { ...e }));
  ctx.events.on('level.up', (e) => void dispatchOutboundPlatformEvent(ctx, 'level.up', e.guildId, { ...e }));
  ctx.events.on('automod.triggered', (e) => void dispatchOutboundPlatformEvent(ctx, 'automod.triggered', e.guildId, { ...e }));
  ctx.events.on('enforcer.decided', (e) => void dispatchOutboundPlatformEvent(ctx, 'enforcer.decided', e.guildId, { ...e }));
}
...
export function createIntegrationsService(ctx: PluginContext): IntegrationsService {
  function resolveArgs(guildIdOrJob: string | BotActionJobShape, endpointId?: string): { guildId: string; endpointId: string } {
  ...
  return {
    async sendOutbound(guildIdOrJob, endpointIdArg, payloadArg) {
      const { guildId, endpointId } = resolveArgs(guildIdOrJob as string | BotActionJobShape, endpointIdArg as string | undefined);
      ...
      const payload = isBotActionJobShape(guildIdOrJob) ? { event: 'integration.manual', guildId, timestamp: new Date().toISOString() } : payloadArg;
      await ctx.queue('outbound').add('deliver', { endpointId: endpoint.id, guildId, payload, attempt: 1 }, OUTBOUND_JOB_OPTIONS);
    ...
    async testWebhook(guildIdOrJob, endpointIdArg) {
      ...
      const payload = { event: 'integration.test', guildId, timestamp: new Date().toISOString(), message: 'This is a test delivery from Entrophy.' };
